...
import CountryMissingStats from './CountryMissingStats';
...
const personPath = (person) => `/missing/${person.slug || person.id}`;
...
export default function Hero() {
  ...
  const selectedPeople = useMemo(
    () => demoMissingPersons.filter((person) => person.countryCode === countryCode),
    [countryCode]
  );

  const carouselItems = useMemo(() => {
    return priorityCarouselItems
      .map((item) => {
        if (item.type === 'missing') {
          return {
            ...item,
            person: demoMissingPersons.find((person) => person.id === item.personId),
          };
        }
      ...
      .filter((item) => {
        if (item.type === 'missing') return item.person && item.person.countryCode === countryCode;
  ...
  const regularPeople = Array.from({ length: Math.min(4, selectedPeople.length) }, (_, index) => selectedPeople[(activeIndex + index + 1) % selectedPeople.length]).filter(Boolean);
  ...
  return (
    <section className="relative isolate w-full overflow-hidden bg-[linear-gradient(135deg,#ffffff_0%,#f0fdf4_58%,#fff8d7_100%)]">
      ...
      <div className="container-hero grid items-center gap-10 py-12 sm:py-16 lg:grid-cols-[minmax(0,0.82fr)_minmax(650px,980px)] lg:gap-12 lg:py-20 xl:gap-16">
        <div className="max-w-[780px] justify-self-start">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/90 px-4 py-2 text-xs font-black uppercase tracking-wide text-emerald-800 shadow-sm ring-1 ring-emerald-100 backdrop-blur">
            ...
            {country.flag} {country.name} public safety platform
          </div>

          <h1 className="mt-6 text-4xl font-black leading-[1.02] tracking-[-0.055em] text-slate-950 sm:text-5xl lg:text-6xl xl:text-7xl">
            Faster missing-person and emergency alerts across {country.name}.
          </h1>

          <p className="mt-5 max-w-2xl text-base leading-8 text-slate-600 sm:text-lg">
            876Alert helps families, communities, and responders in {country.name} share trusted reports, emergency updates, regional notices, and safety resources in one modern platform.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-2.5">
            {['Verified reports', `${country.regions.length} regions`, 'Mobile emergency access'].map((item) => (
          ...
          <dl className="mt-9 grid max-w-2xl grid-cols-3 gap-3">
            {[
              [country.totalMissingReports.toLocaleString(), 'Reports'],
              [selectedPeople.filter((person) => person.status === 'located').length.toLocaleString(), 'Located'],
              ['24/7', 'Alerts'],
            ].map(([value, label]) => (
        ...
        <div className="grid w-full max-w-[980px] gap-4 justify-self-center xl:grid-cols-[minmax(0,700px)_minmax(230px,260px)] xl:items-start lg:justify-self-end">
          <div
                    ...
                    <div key={activeItem.id} className="priority-slide absolute inset-0 flex flex-col overflow-hidden rounded-[1.6rem] bg-gradient-to-br from-red-700 via-red-600 to-rose-700 p-3 text-white shadow-[0_20px_50px_rgba(185,28,28,0.28)]">
                      <div className="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-2 px-1">
                        <div>
                          ...
                          <p className="mt-1 text-sm font-bold text-white/85">{country.flag} {country.name} · Featured / Sponsored</p>
                        </div>
                          ...
                          <div className="relative h-full min-h-[250px]">
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-red-950/95 to-transparent p-4">
                              ...
                              <p className="mt-1 text-sm font-semibold text-white/85">
                                {priorityPerson.age} years · {priorityPerson.region}
                              </p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex min-h-0 flex-col justify-between rounded-[1.35rem] bg-white p-4 text-slate-950 shadow-lg">
                          <div>
                            <div className="flex flex-wrap gap-2">
                              ...
                              <span className="badge bg-yellow-100 text-yellow-900 ring-1 ring-yellow-200">{priorityPerson.flag} {priorityPerson.country}</span>
                            </div>

                            <div className="mt-5 space-y-4 text-sm">
                              <div>
                                <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Region / Location</p>
                                <p className="mt-1 font-black text-slate-950">{priorityPerson.region}</p>
                    ...
                    <div key={activeItem.id} className="priority-slide absolute inset-0 flex flex-col overflow-hidden rounded-[1.6rem] bg-gradient-to-br from-emerald-700 via-emerald-600 to-yellow-400 p-3 text-white shadow-[0_20px_50px_rgba(0,155,58,0.22)]">
                      <div className="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-2 px-1">
                        <div>
                          ...
                          <p className="mt-1 text-sm font-bold text-white/90">{activeItem.flag} {activeItem.country} · {activeItem.badge}</p>
                        </div>
                        ...
                        <div className="min-h-0 overflow-hidden rounded-[1.35rem] bg-emerald-950/20 ring-1 ring-white/15">
                          <div className="relative h-full min-h-[250px]">
                            ...
                            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-emerald-950/95 to-transparent p-4">
                              ...
                              <p className="mt-1 text-sm font-semibold text-white/85">{activeItem.region}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex min-h-0 flex-col justify-between rounded-[1.35rem] bg-white p-4 text-slate-950 shadow-lg">
                          <div>
                            <div className="flex flex-wrap gap-2">
                              ...
                              <span className="badge bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">{activeItem.flag} {activeItem.country}</span>
                            </div>
                            ...
                            <div className="mt-5 rounded-2xl bg-yellow-50 p-4 ring-1 ring-yellow-100">
                              <p className="text-xs font-black uppercase tracking-[0.16em] text-yellow-800">Location / Region</p>
                              <p className="mt-1 font-black text-slate-950">{activeItem.region}</p>
                            </div>
                          </div>
                      ...
                      <div className="grid grid-cols-[5.25rem_1fr]">
                        ...
                        <div className="min-w-0 p-3">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="truncate text-[0.65rem] font-black uppercase tracking-wide text-slate-500">{person.flag} Community Report</span>
                          </div>
                          <h3 className="mt-1 truncate text-sm font-black text-slate-950">{person.name}</h3>
                          <p className="truncate text-xs font-bold text-emerald-700">{person.region}</p>
                          <div className="mt-1 flex items-center justify-between gap-2">
                            <p className="truncate text-[0.68rem] font-black capitalize text-slate-500">{person.status} alert</p>
                            <Link to={personPath(person)} className="rounded-full border border-slate-300 bg-white px-2 py-1 text-[0.64rem] font-black text-slate-800 shadow-sm transition hover:border-emerald-400 hover:text-emerald-700">
                              Details
                            </Link>
                          </div>
                        </div>
                ...
                <div className="mt-4 flex min-h-[10px] items-center justify-center gap-2">
                  {carouselItems.map((item, index) => (
                    <button
                      ...
                      className={`h-2.5 rounded-full transition-all ${index === activeIndex ? 'w-8 bg-emerald-600' : 'w-2.5 bg-slate-300 hover:bg-slate-400'}`}
                      aria-label={`Show ${item.type === 'missing' ? item.person.name : item.businessName}`}
                    />
                  ))}
                </div>
              </>
          ...
          <CountryMissingStats activeIndex={activeIndex} selectedCountryCode={countryCode} />
        </div>
      </div>
    </section>
  );
