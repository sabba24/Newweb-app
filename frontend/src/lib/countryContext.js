...
function createDefaultContext(name) {
  return {
    regions: ['Central Region', 'Northern Region', 'Southern Region', name],
    cities: [name, 'Capital City', 'Metro Area'],
  };
}

export const SUPPORTED_COUNTRIES = countryRows.map(([code, name, flag, phoneCode], index) => {
  const context = contextByCode[code] || createDefaultContext(name);
  ...
  return {
    ...
    flag,
    phoneCode,
    totalMissingReports: totalsByCode[code] || 100 + index * 37,
...
export function getCountryByCode(code) {
  return SUPPORTED_COUNTRIES.find((country) => country.code === code) || SUPPORTED_COUNTRIES.find((country) => country.code === 'JM') || SUPPORTED_COUNTRIES[0];
}
