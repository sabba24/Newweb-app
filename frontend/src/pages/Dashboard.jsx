...
const planLimits = {
  personal_free: { used: 1, limit: 1, label: '1 free alert per month', paid: false },
  personal_pro: { used: 3, limit: 10, label: 'Personal Pro paid alert access', paid: true },
  business: { used: 4, limit: 20, label: 'Business paid alert and promotion access', paid: true },
};
    ...
    <div className="grid gap-5">
      <section className="grid gap-5 xl:grid-cols-[1fr_360px]">
        <div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100 sm:p-8">
          ...
          <h2 className="mt-4 text-2xl font-black tracking-tight text-slate-950">Business safety tools</h2>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">
            Paid business access is ready for sponsored alerts, campaigns, promotions, billing, and website traffic calls-to-action.
          </p>

          <div className="mt-7 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ['Business Ads', 'Sponsored placements coming next'],
              ['Promotions', 'Community safety offers'],
              ['Campaigns', 'Track future campaign activity'],
              ['Website Traffic CTA', 'Send visitors to your business'],
            ].map(([title, text]) => (
              <div key={title} className="rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-100">
        ...
        <div className="rounded-[2rem] bg-emerald-950 p-6 text-white shadow-xl">
          ...
          <h2 className="mt-3 text-2xl font-black">Promote safety services</h2>
          <p className="mt-3 text-sm font-semibold leading-6 text-emerald-50/80">
            Billing is not connected yet, but the business dashboard foundation is ready.
          </p>
          <Link to="/dashboard/business-ads" className="btn mt-5 bg-white text-slate-950">Manage ads</Link>
        </div>
      </section>
    </div>
  );
    ...
    <div className="grid gap-5">
      <section className="grid gap-5 xl:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-[2rem] bg-emerald-950 p-6 text-white shadow-xl">
          ...
          <h2 className="mt-3 text-2xl font-black">{accountType === 'personal_pro' ? 'Paid account active' : 'Unlock more alert tools'}</h2>
          <p className="mt-3 text-sm font-semibold leading-6 text-emerald-50/80">
            {accountType === 'personal_pro'
              ? 'Your paid personal dashboard is ready for expanded alert posting when backend billing is connected.'
              : 'Paid plans for Pro accounts are prepared for future billing integration.'}
          </p>
